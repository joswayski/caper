...
export default function MicPlayback({ stream, output, status }: { stream: MediaStream; output: string; status?: string }) {
  ...
  useEffect(() => {
    ...
    const play = async () => {
      try {
        ...
        if (!current) return;
        if (reason instanceof DOMException && reason.name === "NotAllowedError") setPlaybackStatus("ready");
        else setDeviceError(true);
      }
    };
    void play();
  ...
  useEffect(() => {
    if (!url || !playbackRef.current?.setSinkId) return;
    void playbackRef.current.setSinkId(output).then(() => setDeviceError(false)).catch(() => setDeviceError(true));
  }, [output]);
  ...
  return <section className="mic-test-card" aria-labelledby="mic-test-heading">
    ...
    {url && <div className="mic-playback">
      ...
      <audio ref={playbackRef} aria-label="Recorded microphone test" controls src={url} onPlay={() => setPlaybackStatus("playing")} onEnded={() => setPlaybackStatus("ready")} />
