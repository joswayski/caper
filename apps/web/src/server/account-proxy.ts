// Narrow transport shared by browser session and native token routes. Rust is
// the authority for JWT verification, account ownership, and profile validation.
export async function proxyAccount(request: Request, accountToken: string, operation: "me" | "profile"): Promise<Response> {
  const headers = { "cache-control": "no-store" };
  if (!accountToken) return new Response(null, { status: 401, headers });
  if (request.method !== (operation === "me" ? "GET" : "POST")) return new Response(null, { status: 405, headers });
  ...
  try {
    const response = await fetch(`${base.replace(/\/$/, "")}/api/account/${operation}`, {
      method: request.method,
      headers: { "content-type": "application/json", "x-caper-account-token": accountToken },
