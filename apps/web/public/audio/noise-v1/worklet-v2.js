...
class NoiseProcessor extends AudioWorkletProcessor {
  constructor(options) {
    ...
    this.stopped = false;
    try {
      // v2: the engines run at 48 kHz; CaperRateBridge (resampler-v1, added to
      // the same AudioWorklet first) converts at any other context rate.
      this.bridge = sampleRate === 48000 ? undefined : new globalThis.CaperRateBridge(sampleRate);
      this.core = (input, output) => this.render(input, output);
  ...
  process(inputs, outputs) {
    ...
    try {
      if (this.bridge) this.bridge.process(input ?? this.silence(output.length), output, this.core);
      else this.render(input, output);
    } catch {
      output.fill(0);
      this.stopped = true;
      this.rnnoise?.destroy();
  ...
  silence(length) {
    if (this.zeros?.length !== length) this.zeros = new Float32Array(length);
    return this.zeros;
  }

  /** One output sample per input sample, at 48 kHz; throws on engine failure. */
  render(input, output) {
