...
This is **development calling, not accepted physical-device support**. The UI fixture deliberately returns media 503 and cannot validate Cloudflare. Required acceptance remains: exact-head Apple compilation/tests, two-party Cloudflare calls, TURN-only/network handoff, route/interruption recovery, and sustained locked-iPhone capture/playback on physical hardware. iOS uses Apple's communication-route UI; macOS follows System Settings because the pinned WebRTC binary exposes no safe per-device switch. Output gain is persisted and per-participant local mute/gain is implemented. There is no incoming-call PushKit/CallKit behavior because the server has no incoming-call signaling.

macOS audio preferences include an explicitly started, prejoin-only local microphone recording (up to 30 seconds), natural replay, and a separate EQ comparison. Dismissal or joining closes the test and removes the temporary recording. The EQ changes local replay only: it is **not** web-equivalent live microphone processing and does not feed the WebRTC track. The sheet also shows aggregate connection statistics without addresses, candidate IDs, or credentials. Actual recording/playback and physical routing still require device acceptance. Input gain, in-call mic monitoring, and the web live-processing chain remain unsupported by this build.
