...
There is no incoming-call push, ringing, or invitation UI because the server has no push/incoming-call contract. Camera and screen sharing are not working web features and are not exposed. Input-device selection, mic test/processing preferences, participant volume controls, and diagnostics remain parity gaps. None are represented by inert controls.
